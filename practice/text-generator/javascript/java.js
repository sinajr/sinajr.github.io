...
const dummyText = [
  ...
  let textPlace=$.getElementById('showText');
  let userInpt=$.getElementById('userNum')
  let genBtn=$.getElementById('genBtn')
  console.log(dummyText)
  genBtn.addEventListener('click',textGenerator);
  function textGenerator(){
      const userInptValue=userInpt.value;
      for(let i=0;i<userInptValue;i++){
          let randomNum=Math.floor(Math.random()*8.9);
        let newp=$.createElement('p');
        newp.innerText=dummyText[randomNum];
        textPlace.append(newp);
    }
  }